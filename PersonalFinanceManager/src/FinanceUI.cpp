...
std::string FinanceUI::promptLine(const std::string& message) const
{
    std::cout << message;
    std::string value;
...
std::string FinanceUI::promptOptional(const std::string& message) const
{
    std::cout << message;
    std::string value;
    std::getline(std::cin, value);
...
double FinanceUI::promptAmount(const std::string& message) const
{
    while (true)
    {
...
void FinanceUI::showExpenses(const std::vector<Expense>& expenses) const
{
    if (expenses.empty())
    {
...
void FinanceUI::showIncome(const std::vector<Income>& incomes) const
{
    if (incomes.empty())
    {
...
void FinanceUI::showBudgets(const std::vector<Budget>& budgets) const
{
    if (budgets.empty())
    {
...
void FinanceUI::showSummary(const Summary& summary) const
{
    std::cout << "\n=== Financial Summary ===\n";
    ...
    {
        for (const auto& [category, amount] : summary.spendingByCategory)
        {
    ...
    {
        std::cout << "\nBudgets and status:\n";
        for (const auto& budget : summary.budgets)
        {
            double spent = 0.0;
...
void FinanceUI::filterExpensesFlow() const
{
    ...
    try
    {
        auto filtered = service_.filterExpenses(category, startDate, endDate);
