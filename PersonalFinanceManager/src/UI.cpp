...
void FinanceUI::showExpenses(std::vector<Expense> const& expenses) const
{
    ...
    {
        std::cout << expense.id << " | " << expense.date << " | " << expense.category << " | " << expense.amount << " | ";
...
void FinanceUI::showIncome(std::vector<Income> const& incomes) const
{
    ...
    {
        std::cout << income.id << " | " << income.date << " | " << income.source << " | " << income.amount << " | ";
...
void FinanceUI::showSummary(Summary const& summary) const
{
    ...
    if (!summary.budgets.empty())
    {
        ...
        for (auto const& budget : summary.budgets)
        {
            ...
            std::cout << "  " << budget.category << ": budget " << budget.amount << " per " << budget.period
                      << ", spent " << spent << ", ";
            if (remaining < 0)
            {
...
{
    std::string text = promptLine("Enter expense ID to delete: ");
    try
    {
        int id = std::stoi(text);
        bool removed = service_.deleteExpense(id);
        if (removed)
        {
    ...
    {
        std::cout << "Please enter a valid numeric ID.\n";
    }
}
...
void FinanceUI::run()
{
    while (true)
    {
        ...
        std::cout << "7. Set budget\n";
        std::cout << "8. View budgets\n";
        std::cout << "9. View dashboard\n";
        std::cout << "0. Exit\n";
